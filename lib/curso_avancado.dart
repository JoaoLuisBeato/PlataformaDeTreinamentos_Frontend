...
class CursoAvancadoCall extends StatefulWidget {
  final int randId;
  final String emailUser;
  final int flag;

  const CursoAvancadoCall({required this.randId, required this.emailUser, required this.flag});

  @override
  CursoAvancado createState() => CursoAvancado();
}

class CursoAvancado extends State<CursoAvancadoCall> {
  int randId = 0;
  String emailUser = '';
  int flagToSend = 2;
  ...
  Future<void> fetchDataFromAPI() async {
    final url =
        Uri.parse('http://127.0.0.1:5000/curso_avancado_treinamentos');
    final response =
        await http.post(url, body: {'id': widget.randId.toString()});

    setState(() {
      curso_introdutorio = jsonDecode(response.body);
      curso_introdutorio_texto = curso_introdutorio[0]['ca'];
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
          title: const Text('Curso Avançado'),
      ...
      body: Padding(
          ...
          child: Column(
            children: [
              Center(
                child: Text('Curso Avançado', style: styleMainTitle),
              ),
              SizedBox(height: 50),
              Center(
                child: Expanded(
                ...
                onPressed: () {
                  showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          title: const Text(
                              'Deseja ir para o teste do curso avançado?'),
                          content:
